        ...
        while unvisited:
            ...
            for neighbour, _ in self.graph.get(min_vertex, {}).items():
                if neighbour in visited:
                    continue
                new_distance = unvisited[min_vertex] + self.graph[
                    min_vertex].get(neighbour, float("inf"))
...
class PlayerA(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.focus_coin = None  # I added this to keep track of sub goal
        self.actions = []  # Keep track of current plan
        self.speedx = SPEED
        self.speedy = SPEED
        self.score = 0
        self.steps = 0
    ...
    def update(self):
        ############
        ### TODO: Please implement your algorithm (e.g. how your agents decide to move)
        ### Based on the informatino of the Coins and Walls
        ############
        ...
        # there is no plan, just go for a coin
        # OR if the coin we were focusing on is now gone
        focus_coin_gone = (self.focus_coin is not None and
                           self.focus_coin not in coin_positions)
        if len(self.actions) == 0 or focus_coin_gone:
            if focus_coin_gone:
                print('Agent 1 is planning a new path because the focus coin '
                      'is gone')
            else:
                print('Agent 1 is planning a new path')
            exclude_coin_values = []
            exclude_coin_positions = []
            while self.actions is None or len(self.actions) == 0:
                whitelist_coin_values = minus_lists(
                    coin_values,
                    exclude_coin_values
                )
                whitelist_coin_positions = minus_lists(
                    coin_positions,
                    exclude_coin_positions
                )
                nearest_coin_pos = find_nearest_coin(
                    [self.true_x, self.true_y],
                    whitelist_coin_values,
                    whitelist_coin_positions
                )
                self.focus_coin = nearest_coin_pos
                path = path_find(
                    1,
                    [self.true_x, self.true_y],
                    nearest_coin_pos,
                    ...
                    whitelist_coin_positions,
                    wall_positions,
                    [-2, -2]  # we're just going to ignore the other player's pos
                )
                self.actions = convert_path_to_actions(path)
                # add nearest coin to exclude to ignore if there is no path to
                # it and we need to do another iteration of searching for the
                # nearest coin
                exclude_coin_values.append(
                    # since nearest_coin_pos was searched in the whitelist
                    # we have to search for its position relative in our
                    # whitelisted coin positions list, and then search that in
                    # our whitelisted coin values lists
                    whitelist_coin_values[
                        whitelist_coin_positions.index(
                            nearest_coin_pos
                        )
                    ]
                )
                exclude_coin_positions.append(nearest_coin_pos)
            assert len(self.actions) > 0, 'Could not find a path to a near coin'
            # this error should not happen^
            # and it did not happen in my tests

        # only other reconsideration (the only explicit reconsideration):
        # if we are next to another player, run away one step

        # i actually can't reliably extract the other players position, so i'm
        # just going to say i would've done this if i could
        ...
        if len(self.actions) == 0 or focus_coin_gone:
            ...
            while self.actions is None or len(self.actions) == 0:
                ...
                path = path_find(
                    ...
                    wall_positions,
                    [-2, -2]
                    # we're just going to ignore the other player's pos
                )
                self.actions = convert_path_to_actions(path)
                # add nearest coin to exclude to ignore if there is no path to
                # it and we need to do another iteration of searching for the
